...
configFrom lsColors =
  Config
    { leftEscapeSequence = Maybe.fromMaybe (leftEscapeSequence def) $ "lc" `Map.lookup` parametors,
      rightEscapeSequence = Maybe.fromMaybe (rightEscapeSequence def) $ "rc" `Map.lookup` parametors,
      endEscapeSequence = Maybe.fromMaybe (endEscapeSequence def) $ "ec" `Map.lookup` parametors,
      resetEscapeSequence = Maybe.fromMaybe (resetEscapeSequence def) $ "rs" `Map.lookup` parametors,
      normalEscapeSequence = Maybe.fromMaybe (normalEscapeSequence def) $ "no" `Map.lookup` parametors,
      fileEscaseSequence = Maybe.fromMaybe (fileEscaseSequence def) $ "fi" `Map.lookup` parametors,
      ...
      missingFileEscapeSequence = Maybe.fromMaybe (missingFileEscapeSequence def) $ "mi" `Map.lookup` parametors,
      ...
      otherWritableEscapeSequence = Maybe.fromMaybe (otherWritableEscapeSequence def) $ "ow" `Map.lookup` parametors,
      stickyOtherWritableEscapeSequence = Maybe.fromMaybe (stickyOtherWritableEscapeSequence def) $ "tw" `Map.lookup` parametors,
      capabilityEscapeSequence = Maybe.fromMaybe (capabilityEscapeSequence def) $ "ca" `Map.lookup` parametors,
      multiHardlinkEscapeSequence = Maybe.fromMaybe (multiHardlinkEscapeSequence def) $ "mh" `Map.lookup` parametors,
      clearLineEscapeSequence = Maybe.fromMaybe (clearLineEscapeSequence def) $ "cl" `Map.lookup` parametors,
...
extensionConfigFrom lsColors =
  ExtensionConfig
    { userReadPermBitEscapeSequence = Maybe.fromMaybe (userReadPermBitEscapeSequence def) $ "ur" `Map.lookup` parametors,
      ...
      otherWritePermBitEscapeSequence = Maybe.fromMaybe (otherWritePermBitEscapeSequence def) $ "tw'" `Map.lookup` parametors, -- WIP: `tw` is conflict between GNU LS and EXA
      ...
      fileInodeEscapeSequence = Maybe.fromMaybe (fileInodeEscapeSequence def) $ "in" `Map.lookup` parametors
    }
  where
